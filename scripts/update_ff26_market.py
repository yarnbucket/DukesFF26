...
def build_market(players_payload: dict, old_market: dict, payloads: tuple[object, object, object, object]):
    ...
    if len(tier_records) < 100:
        raise RuntimeError(f"FantasyPros tier response was unexpectedly small ({len(tier_records)} players).")
    if len(p25_records) < 100 or len(p24_records) < 100:
        raise RuntimeError("FantasyPros historical player-points response was unexpectedly small.")
    ...
    for player in player_list:
        ...
        if games26 > 0 and avg26 is not None and avg25 is not None:
            ...
            current_season = previous_season = None
            current_ppg = previous_ppg = None
            current_games = previous_games = 0

        trend = (
            round(current_ppg - previous_ppg, 1)
        ...
        out_players[name] = {
            ...
            "trend": trend,
            "trendSource": "FANTASYPROS_PPR_PLAYER_POINTS" if trend is not None else "UNAVAILABLE",
            "currentSeason": current_season,
            "previousSeason": previous_season,
            "currentPpg": round(current_ppg, 2) if current_ppg is not None else None,
            "previousPpg": round(previous_ppg, 2) if previous_ppg is not None else None,
            "currentGames": current_games,
            "previousGames": previous_games,
        }
